...
<body>
  ...
  <tbody><tr>
    ...
    <select name="category"  class="dfinput">
      <?php if(is_array($channelList)): $i = 0; $__LIST__ = $channelList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($info['category'] == $vo['id']): ?>selected<?php endif; ?> ><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
    </select>
    </td>
  ...
  <tr>
    <td>描述</td>
    <td colspan="4"><textarea class="dfinput" name="description" style="width:82%; height:50px;" ><?php echo ($info["description"]); ?></textarea></td></tr>
  <tr>
    <td>内容</td>
    <td colspan="5">
      <script type="text/javascript">
