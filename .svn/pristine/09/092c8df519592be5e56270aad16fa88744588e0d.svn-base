...
<header style="background:url('/Public/Admin/img/topbg.gif') repeat-x; height:88px;">
    ...
    <ul class="nav" id="nav">
        <?php if(is_array($navList)): $i = 0; $__LIST__ = $navList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li onclick="_M(<?php echo ($vo["id"]); ?>,'/admin.php/<?php echo ($vo["name"]); ?>')"><?php echo ($vo["title"]); ?></li><?php endforeach; endif; else: echo "" ;endif; ?>
    </ul>
    <div class="topright">    
    <ul>
...
<div id="content">
  ...
  function _M(menuid,targetUrl) {
     var menuHtml="";
     $.ajax({
        url:"/admin.php/Public/Left/mid/"+menuid,
        type:"POST",
        success:function(data){
            menuHtml+="<div class=\"lefttop\"><span></span>"+data['menu']+"</div><ul class=\"menuson\">";
            $.each(data,function(i) { 
                if(data[i].title){
                   menuHtml +="<li onclick=\"_MP("+data[i].id+",'/admin.php/"+data[i].name+"/"+data[i].action+"?cid="+data[i].id+"')\"><cite></cite>"+data[i].title+"<i></i></li>";
                } 
            });
            menuHtml+="</ul>";
