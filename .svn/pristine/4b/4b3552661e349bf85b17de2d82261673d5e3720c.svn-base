...
<header style="background:url('/Public/Admin/img/topbg.gif') repeat-x; height:88px;">
    ...
    <ul class="nav" id="nav">
        <?php if(is_array($navList)): $i = 0; $__LIST__ = $navList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li onclick="_M(<?php echo ($vo["id"]); ?>,'/index.php/Admin/<?php echo ($vo["module"]); ?>')"><?php echo ($vo["title"]); ?></li><?php endforeach; endif; else: echo "" ;endif; ?>
    </ul>
    <div class="topright">    
    <ul>
        ...
        success:function(data){
            ...
            $.each(data,function(i) { 
                if(data[i].title){
                   ...
                   menuHtml +="<li onclick=\"_MP("+data[i].id+",'/index.php/Admin/"+data[i].module+"')\"><cite></cite>"+data[i].title+"<i></i></li>";
                } 
            });
            menuHtml+="</ul>";
