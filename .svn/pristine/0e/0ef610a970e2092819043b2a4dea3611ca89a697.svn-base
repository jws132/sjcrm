                                        ...
                                        <div class="controls">
                                              <select name="pid" id="selectId">   
                                                   ...
                                                   <?php if(is_array(C("PRO_LIST_TYPE"))): $k = 0; $__LIST__ = C("PRO_LIST_TYPE");if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$newsType): $mod = ($k % 2 );++$k;?><option value="<?php echo ($k); ?>" disabled><?php echo ($newsType); ?></option>
                                                      <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i; if($l['type'] == $k): ?><option value="<?php echo ($l["id"]); ?>" typeid="<?php echo ($k); ?>" <?php if($info['pid'] == $l['id']): ?>selected<?php endif; ?>>&nbsp;&nbsp;&nbsp;|-<?php echo ($l["title"]); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; endforeach; endif; else: echo "" ;endif; ?>
                                              </select><input type="hidden" name="type" id="type" value="<?php echo ($info["type"]); ?>">       
                                        </div>
                                    </div>
                                    <div class="control-group">
                                        <label class="control-label">编号</label>
                                        <div class="controls">
                                            <input type="text" name="number" value="<?php echo ((isset($info["number"]) && ($info["number"] !== ""))?($info["number"]):''); ?>" style="width:480px;">
                                        </div>
                                    </div>
                                    <div class="control-group">
                                        <label class="control-label">名称</label>
                                        <div class="controls">
                                            <input type="text" name="title" value="<?php echo ((isset($info["title"]) && ($info["title"] !== ""))?($info["title"]):''); ?>" style="width:480px;">
                                        </div>
                                    </div>
                                    <div class="control-group">
                                        <label class="control-label">规格</label>
                                        <div class="controls">
                                            <input type="text" name="spec" value="<?php echo ((isset($info["spec"]) && ($info["spec"] !== ""))?($info["spec"]):''); ?>" style="width:480px;">
                                        </div>
                                    </div>
                                     <div class="control-group">
                                        <label class="control-label">备注</label>
                                        <div class="controls">
                                            <textarea name="des" style=" width:480px;"><?php echo ((isset($info["des"]) && ($info["des"] !== ""))?($info["des"]):''); ?></textarea>
                                        </div>
                                    </div>
                                    <div class="control-group">
